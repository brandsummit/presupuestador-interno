...
function getPaymentItems(quote: Quote) {
  return (quote.summary_payment_items || [])
...
export default function ProposalSummary({ quote }: Props) {
  ...
  const paymentItems = getPaymentItems(quote);
  const paymentPercentages = getPaymentPercentages(paymentItems.length);
  ...
  return (
    <section className="px-10">
      ...
      <div className="mt-24 grid items-stretch gap-4 xl:grid-cols-3">
        <article className="flex flex-col justify-between gap-8 rounded-[36px] bg-prop-text p-10 text-prop-background xl:rounded-l-[36px] xl:rounded-r-xl">
          ...
          <div>
            <h4 className="text-base font-bold">{t.summary.total}</h4>
        ...
        <article className="flex flex-col justify-between gap-8 rounded-[36px] border border-prop-text/60 p-10 xl:rounded-xl">
          <div className="grid items-stretch gap-4 lg:grid-cols-2">
            <div
              className={`
                flex h-full flex-col justify-between gap-8 rounded-xl
                bg-prop-text p-6 text-prop-background
                ${paymentItems.length <= 1 ? "min-h-64" : ""}
              `}
            >
              <p className="font-display text-6xl leading-none">50%</p>

              <p className="text-base">{t.summary.projectStart}</p>
            </div>
        ...
        <article className="flex flex-col justify-between gap-8 overflow-hidden rounded-[36px] bg-prop-text text-prop-background xl:rounded-l-xl xl:rounded-r-[36px]">
          ...
          <div className="p-10 pt-0">
            <h4 className="text-base font-bold">{t.summary.startDate}</h4>
